...
class Pomodoro():
    ''' My implementation of a Pomodoro timer.It's a time management tool that breaks work into intervals separated by
        breaks, some longer some shorter.
    '''
    ...
    def done(self):
        '''
            Denotes if the Pomodoro task goal has been met.
...
def main():
    ...
    while True:
        ...
        if pomodoro.timer_type == Pomodoro.TIMER_TASK:
            ...
            # check to see if you need to continue to a break or stop the pomodoro
            if pomodoro.done():
                break
